...
xref:CHANGELOG.md[Change Log] | xref:docs/doc.adoc[Full Documentation]

A RISC-V 32-bit System-on-a-Chip written in VHDL targeted
for an FPGA.

== Quick info

This RISC-V SoC uses the RV32IM instruction set
and the Zicsr, Zicntr, Zicond, Zimop, Zihpm, Zba, Zbb, Zbs,
Zbkb, Sdext and Sdtrig extensions. The SoC is
build around a three-stage pipelined core.
...
SoC supports Machine mode only. Traps (interrupts
...
clocks, the SoC does not support branch prediction.
All other instructions require 1 clock.

Current Coremark testbench shows a CPI of 1.75 and a score
of 1.94 coremark/MHz (GCC 16.1, -O3).
...
(There is also a https://github.com/jesseopdenbrouw/thuas-riscv-5stage[5-stage pipelined SoC].)
 
== Memory

The SoC uses FPGA onboard RAM blocks to emulate RAM
...
* BOOT: a bootloader ROM of 4 kB (placed in onboard RAM, fixed).
...
Lite 25.1. Simulation is possible with QuestaSim Intel Starter Edition.
...
(Spartan 7 boards) and a https://www.terasic.com.tw/cgi-bin/page/archive.pl?Language=English&CategoryNo=234&No=1021[DE10-Lite] board.
...
The SoC can be equipped with an on-chip debugger that complies to the
https://docs.riscv.org/reference/debug/v1.0/index.html[RISC-V Debug Specification v1.0].
It is an all-hardware solution, there is no program buffer involved
(yes, the program buffer is optional). All processor registers (including
CSRs) can be read and written (when possible). Memory can be read and
written. There is one hardware breakpoint available. The on-chip debugger
...
This SoC is for educational purposes only.
