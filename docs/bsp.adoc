...
* Set the state of an output pin. More than one pin may be set in one call. The state is either `GPIO_PIN_RESET` or `GPIO_PIN_SET`. Note: this is a read-modify-write function and is not atomic.

`void gpioa_togglepin(uint32_t pin)`

* Toggle the state of a output pin. More than one pin may be toggled in one call. Note: this is a read-modify-write function and is not atomic.
...
* When used to set CTRL, this macro sets Master Acknowledge. Needed when receiving data.
...
Note: these functions work with 8-bit data.
