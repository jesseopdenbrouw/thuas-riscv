...
The memory interface uses a simple request/response protocol. When a load or a store is executed, the memory interface puts the address, the data (writes), the data size and the read/write flag on the bus. These signals are held stable until a acknowledge from the memory is sampled, or an exception is generated. If there is no acknoledge nor exception generated, the core will stall permanently. Currenlty a memory access must at least take two clock cycles.
