...
The OCD is an all-hardware solution. Accessing core registers and memory is done by Abstract Commands. There is no support for program buffer (progbuf) and System Bus Access. This means that the debugger cannot execute the `fence` instruction (or any other instruction). That is not a problem, since the core will wait for a memory operation to be completedi before entering denug state. Note that from the debug specification, the program buffer is optional.
