...
The memory interface uses a simple request/response protocol. When a load or a store is executed, the memory interface puts the address, the data (for writes), the data size and the read/write flag on the bus. In the first cycle of the memory access, a strobe signal is present. These signals are held stable until an acknowledge from the memory is sampled, or an exception is generated. If there is neither acknowledge nor exception generated, the core will stall permanently. Currently a memory access must at least take two clock cycles.
