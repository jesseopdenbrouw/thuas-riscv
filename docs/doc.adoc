...
The memory interface uses a simple request/response protocol. When a load or a store is executed, the memory interface puts the address, the data (for writes), the data size and the read/write flag on the bus. The core has internal buffer for all outgoing signals so the operation is visible one clock cycle later. In the first cycle of the memory access, a strobe signal is present. These signals are held stable until an acknowledge from the memory is sampled, or an exception is generated. If there is neither an acknowledge nor an exception generated, the core will stall permanently. Currently a memory access must at least take two clock cycles. See figure.

.Timing diagram of a read operation.
[[fig_read]]
image::images/read.svg[Image of the read operation]

The memories are responsible for handling different data sizes and rearranging half word and byte access. When reading, the core expects half word data on the lower 16 bits of the data bus and bytes on the lower 8 bits of the databus. When writing the core puts half words on the lower 16 bits of the data bus and bytes on the lower 8 bits of the data bus. The memories are also responsible for signaling misaligned accesses. Sign and zero extension is handled by the core. The memories are also responsible for generating access and misaligned exceptions.
