...
When an interrupt occurs, the FSM enters the `trap` state for fetching the trap vector. An interrupt can occur any time and is called asynchronous. When an interrupt occurs, instruction-to-executed is discarded and must be restarted after return. After fetching the trap handler, new instructions are fetched and the instruction pipeline is flushed.
...
Note that _all_ memory accesses must be acknowledged, even when this triggers an exception.
...
Note that *all* memory accesses must be acknowledged, even when this triggers an exception.
...
.FPGA resource utilization for the DE0-CV board (Slow 1100mV 85C model, v1.0.2.0).
[[tab_metrics]]
[cols="1,1,1,1]
|===
| Setup | $f_{max}$ | ALMs | FFs | RAM

| Default {all I/O, MD, no extensions, no OCD} | 88 | 2886 | 2449 | 1,345,536
| Default, no bootloader, no OCD               | 93 | 2904 | 2471 | 788,480
| Default, only UART1 + bootloader, no OCD     | 91 | 2088 | 1458 | 1,345,536
| Small (UART1, no boot, no MD, no OCD)        | 92 | 1699 | 1278 | 788,480 
| Default OCD (no bootloader, OCD)             | 84 | 3465 | 3076 | 1,313,792
| Full OCD (all extensions, OCD)               | 82 | 4503 | 3529 | 1,313,792
|===
...
* `core.vhd` -- The core contains the PC, the registers, the ALU, the MD unit, the control state machine, the memory interface, the CSR, the LICi and debug hardware.
...
When OCD_CSR_CHECK_DISABLE is set to true, accessing unimplemented CSRs do not trigger an exception. This breaks the RISC-V debug spec.
...
----
(The `--recursive` option may be omitted, in which case the `make` process with fetch them.)
