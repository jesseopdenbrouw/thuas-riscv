...
The memories are described in a device agnostic manner. It is up to the synthesizer to map onboard memory to the instantiated  memories. This can lead to excessive on-board RAM usage because the synthesizer is not able to create true dual-port RAM.
...
The Instruction Router routes instructions from the ROM, RAM and the boot ROM. If the boot ROM is disabled, the synthesizer will remove the instruction decoder. In the default setup, instructions cannot be read from the RAM.
...
In the default setup, a write needs two clock cycles and a read needs three clock cycles for ROM, boot ROM and I/O. For I/O, reads and writes take three clock cycles. When the `FAST_MEM` generic is set to true, all accesses are reduced by one clock cycle. This will speed up prohgrams but has an severe inpact on the $f_{max}$.
...
[source,c,subs=attributes+]
----
srec2mif [-vqbhwd] inputfile [outputfile]
----
