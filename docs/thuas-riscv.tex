...
\item The debug unit regulates on-chip debugging.
\end{itemize}
...
\item The debug module (DM) is an interface between the core and the debug module interface (DMI).
\item The debug transport module (DTM) is an interface between the DMI and the external debug hardware.
\end{itemize}

A simplified block diagram of the core is shown in Figure~\ref{fig:blockdiagram}.
...
\node[state,below of=exec] (wfi) {wfi};
...
\path[-latex] (boot0) edge node[above] {--} (boot1)
              ...
              (exec) edge node[left] {wfi} (wfi)
;
\path[-latex,dashed,red]
              (exec) edge node[right] {trap} (trap)
              (wfi) edge node[above] {trap} (trap)
              (mem) edge node[right] {trap} (trap)
;
%\node at (0,-4) {\small * trap initiated by hardware or software.};
\end{tikzpicture}
\caption{FSM of the control of the processor, without on-chip debugging.}
\label{fsmpipe}
\end{figure}
...
\item EBREAK -- break to debugger, debug mpde
...
For on-chip debugging purposes, there are five CSRs:

\begin{itemize}
\item \lstinline|dcsr| -- this register holds information between the DM and de hart.
\item \lstinline|dpc| -- this register hold the PC value of the instruction to be executed when the core enters debug mode.
\item \lstinline|tselect| -- this register is used to select a hardware breakpoint, since there is only one hardware breakpoint available this register is hardwrired to all-zero bits.
\item \lstinline|tdata1| -- this register selects the trigger type for hardware triggering, only type 6 is supported, and only instruction address breakpoint is available.
\item \lstinline|tdata2| -- this register holds the instruction address of the breakpoint.
\item \lstinline|tinfo| -- this register holds additional trigger information.
\end{itemize}
...
\item \lstinline|dm.vhd| -- Description of the Debug Module.
\item \lstinline|dtm.vhd| -- Description of the Debug Transport Module.
...
tck           & P18      & GPIO\_0\_27 , JTAG clock \\
tdi           & R17      & GPIO\_0\_29 , JTAG data in \\
tdo           & T20      & GPIO\_0\_31 , JTAG data out \\
tms           & T18      & GPIO\_0\_33 , JTAG select \\
trst          & T15      & GPIO\_0\_35 , JTAG reset
\end{longtable}
...
\lstinline|HAVE_OCD| & boolean & false & Enable on-chip debugging \\
\lstinline|OCD_CSR_CHECK_DISABLE| & boolean & false & Disable CSR address check \\
...
\lstinline|openocd| -- OpenOCD configuration file +SVD file \\
...
\item \lstinline|uart_interrupt| -- Example program on how to use the UART1 interrupt for transmiiting and receiving data. Works on the board.
...
\item \lstinline|watchdog| -- simple watchdog test program, NMI not yet tested. Works on the board.
\item \lstinline|wfitest| -- simple program to test the \texttt{wfi} instruction. Works on the board.
\end{itemize} 
...
\lstinputlisting[language=C,breaklines=false,linerange={43-50}]{code/riscv.vhd}
...
\lstinputlisting[language=C,breaklines=false,linerange={39-42}]{code/riscv.vhd}
...
\section{Using the on-chip debbuger}
\label{sec:ocd}
\textbf{note: experimental} \\
The on-chip debugger (OCD) follows the guidelines of the RISC-V debug specification version 1.0.0-rc3 (\url{https://github.com/riscv/riscv-debug-spec/releases/download/1.0.0-rc3/riscv-debug-specification.pdf}). The on-chip debugger is compatible with OpenOCD, GDB for RISC-V and Eclipse-CDT. The OCD consists of three parts: the debug module (DM), the debug transport module (DTM) and the trigger module (TM). The DTM is reused from the NEORV32 DTM by Stephan Nolting with his permission (see \url{https://github.com/stnolting/neorv32/blob/main/rtl/core/neorv32_debug_dtm.vhd}). The DM and DTM are external to the core, the TM is incorporated in the core. Features of the OCD are:

\begin{itemize}
\item Standard JTAG access port.
\item Halt/resume the hart,
\item Single-step the hart,
\item Software breakpoints,
\item Hardware breakpoint (one),
\item Read/write all core registers (GPR and CSR),
\item Read/write memory.
\end{itemize}

Accessing core registers and memory is done by Abstract Commands. There is no support for program buffer (progbuf) and System Bus Access. This means that the debugger cannot execute the \lstinline|FENCE| instruction. That is not a problem, since the core will wait for a memory operation to be completed.

From the debug specification, Chapter 3, there is a list of requirements. Some are mandatory, others are optional. All requirements for multiple harts are not applicable. See Table~\ref{tab:req}.

\begin{table}[!ht]
\centering
\caption{List of requirements vor the on-chip debugger.}
\label{tab:req}
...
Required & yes \\
Provide a mechanism to allow debugging harts immediately out of reset (regardless of the reset
cause). & Optional & yes \\
...
\paragraph{Reset Control} The hart can be reset, even from the very first instruction. In OpenOCD, just issue the command \lstinline|reset halt| and the hart will be reset and halted. Note that registers and memory retain their last committed value. Resetting the hart takes a few clock cycles.

\paragraph{Run Control} To resume the hart, issue the OpenOCD command \lstinline|resume|. To halt the hart, issue the OpenOCD command \lstinline|halt|. Note that the \lstinline|cycle| and \lstinline|time| counters keep on running in the halted state.

\paragraph{Single-stepping} When the hart is halted, use the OpenOCD command \lstinline|step| to execute exactly one instruction. Note that interrupts are blocked while single-stepping. Note that the \lstinline|cycle| and \lstinline|time| counters keep on running.

\paragraph{Software Breakpoints} The OCD can insert instruction address breakpoints and run a hart until a breakpoint is hit. This is done by replacing the instruction at the breakpoint address with the \lstinline|EBREAK| instruction. As such, the \lstinline|EBREAK| instruction can not be used in user programs. When the breakpoint is hit, the hart enters debug mode. To set up a software breakpoint, use the OpenOCD command \lstinline|bp <address> 4|.

\paragraph*{Hardware Breakpoint} the hart is equipped with one hardware breakpoint. This is useful when the ROM is read-only. When hitting the hardware breakpoint, The core enters debug mode. To set up a hardware breakpoint, use the OpenOCD command \lstinline|bp <address> 4 hw|.

\paragraph*{\lstinline|WFI| Instruction}
When the hart is halting when executing the \lstinline|WFI| instruction, the hart enters debug mode. Then stepping, the \lstinline|WFI| instruction is treated as a \lstinline|NOP| instruction.

To use the OCD, the VHDL generic \lstinline|HAVE_OCD| must be set to true. The debug specification states that accessing an unimplemented register, the DM must report an error. This is very annoying when using Eclipse-CDT. The VHDL generic \lstinline|OCD_CSR_CHECK_DISABLE|, when set to true, disables this checking. This will break the debug specification. When using OCD, the bootloader is usually disabled.

Note: the OCD is highly experimental. We tested the OCD with OpenOCD, GDB and Eclipse-CDT. When there are issues, please report them to \href{mailto:J.E.J.opdenBrouw@hhs.nl}{\texttt{J.E.J.opdenBrouw@hhs.nl}}.

Tests with the OCD were conducted with an FT2232H MINI MODULE. See \url{https://ftdichip.com/products/ft2232h-mini-module/}. The project's \lstinline|openocd| directory contains a startup script for OpenOCD as well as a SVD-file for GDB en Eclipse-CDT.


\section{Support for Windows}

There is support for Windows. Both \lstinline|srec2vhdl| and \lstinline|upload| can be build with GCC MinGW and Visual Studio.For building the RISC-V programs, a RISC-V GNU GCC compiler is needed.

Best is to use a pre-compiled compiler for Windows and build tools (\lstinline|make|, \lstinline|rm|, \lstinline|mkdir| etc.). Please have a look at \url{https://xpack.github.io/dev-tools/riscv-none-elf-gcc/}
and \url{https://xpack.github.io/dev-tools/windows-build-tools/}.
For building \lstinline|srec2vhdl| and \lstinline|upload|, you need a GCC native compiler. Have a look
at \url{https://xpack.github.io/dev-tools/gcc/}. OpenOCD can be found at \url{https://xpack-dev-tools.github.io/openocd-xpack/}.
