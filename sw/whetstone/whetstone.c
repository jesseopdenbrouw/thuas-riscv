...
main(void)
{
    ...
    uart1_printf("\r\n");
    uart1_printf("##########################################\r\n");
    uart1_printf("%s Precision C Whetstone Benchmark Version 1.2	22 March 1998\r\n", Precision);
...
C
*/
    startsec = csr_get_time();
    start_cycle = csr_get_cycle();
...
C
*/
    end_cycle = csr_get_cycle();
    end_instret = csr_get_instret();
    finisec = csr_get_time();
...
*/
    ...
    if (finisec-startsec <= 0) {
        uart1_printf("Insufficient duration- Increase the LOOP count\r\n");
    }

	uart1_printf("Loops: %ld, Iterations: %d, Duration: %ld usec.\r\n",
			LOOP, II, finisec-startsec);

    if ((finisec - startsec) > 0) {
        KIPS = (100.0*LOOP*II)/( (float)(finisec-startsec) / 1000000.0);
    } else {
        KIPS = ((100.0*LOOP*II)/(float)used_cycle) * csr_read(0xfc1);
    }
    mwips = KIPS/1000.0;
	uart1_printf("C Converted %s Precision Whetstones: %.3f MIPS\r\n", Precision, mwips);
    mwips_mhz = mwips / csr_read(0xfc1) * 1000000;

    uint32_t whet_mwips = (uint32_t)(mwips_mhz * 1000);
    char *pstr = dec2str(whet_mwips);
    uart1_printf("\nCSV, Benchmark, MWIPS/MHz-v1.2\r\n");
    uart1_printf("CSV, Whetstone_v1.2, %u.%s\r\n", (unsigned int)(whet_mwips/1000), pstr);

    float f_ipc = (((float)used_instret / used_cycle));
    uint32_t i_ipc = (uint32_t)(f_ipc * 1000);
    pstr = dec2str(i_ipc);

    uart1_printf("IPC = Instret/Cycle = %u/%u = %u.%s\r\n", (unsigned int)used_instret, (unsigned int)used_cycle, (unsigned int)(i_ipc/1000), pstr);
...
{
	uart1_printf("%7ld %7ld %7ld %12.4e %12.4e %12.4e %12.4e\r\n",
						N, J, K, X1, X2, X3, X4);
}
#endif
